...
##################### CHANGES MADE ############################
# increased runtime metrics for SamSplitter task
###############################################################
...
task SamSplitter {
  ...
  Float disk_multiplier = 2.5
  Int disk_size = ceil(disk_multiplier * unmapped_bam_size + 50) # + 20 -> + 50
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "6 GB" # 3.75 -> 6 GB
    disk: disk_size + " GB"
    cpu: 10 # added cpu specification 
  }
}

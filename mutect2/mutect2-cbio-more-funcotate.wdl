...
## 
## LAST UPDATED: August 22, 2023 - increased runtime metrics for Funcotate task (Kelly Zhang)
...
task Funcotate {
     ...
     Int disk = select_first([disk_space, runtime_params.disk]) + 10
     Int memory = runtime_params.gatk_docker + 50
    ...
    runtime {
        docker: runtime_params.gatk_docker
        #bootDiskSizeGb: runtime_params.boot_disk_size
        memory: memory + " MB" # made memory variable, which adds 50 MB
        disk: disk + " GB" # made disk variable, which adds 10 GB
        preemptible: true
        maxRetries: runtime_params.max_retries
        cpu: 10 # 6 -> 10
    }

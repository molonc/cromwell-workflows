...
import "https://raw.githubusercontent.com/jliebe-bccrc/cromwell-workflows/main/ubam-pre-pro/tasks/Alignment.wdl" as Alignment
import "https://raw.githubusercontent.com/aparicio-bioinformatics-coop/cromwell-workflows/main/ubam-pre-pro/tasks/MarkDuplicatesMemory-BamProcessing.wdl" as Processing # link to increased memory wdl
import "https://raw.githubusercontent.com/jliebe-bccrc/cromwell-workflows/main/ubam-pre-pro/tasks/Utilities.wdl" as Utils
import "https://raw.githubusercontent.com/jliebe-bccrc/cromwell-workflows/main/ubam-pre-pro/tasks/Qc.wdl" as QC
import "https://raw.githubusercontent.com/jliebe-bccrc/cromwell-workflows/main/ubam-pre-pro/tasks/SplitLargeReadGroup.wdl" as SplitRG 
import "https://raw.githubusercontent.com/jliebe-bccrc/cromwell-workflows/main/ubam-pre-pro/tasks/GermlineStructs.wdl" as Structs
...
workflow UnmappedBamToAlignedBam {
  ...
  # Align unmapped input bams in parallel
  scatter (unmapped_bam in unmapped_bams) {

    Float unmapped_bam_size = size(unmapped_bam, "GB")

    String unmapped_bam_basename = basename(unmapped_bam, sample_info.unmapped_bam_suffix)
  ...
  call Processing.MarkDuplicates as MarkDuplicates {
    input:
      ...
      output_bam_basename = sample_info.base_file_name + ".aligned.unsorted.duplicates_marked",
      metrics_filename = sample_info.base_file_name + ".duplicate_metrics",
  ...
  call Processing.SortSam as SortSampleBam {
    input:
      ...
      output_bam_basename = sample_info.base_file_name + ".aligned.duplicate_marked.sorted",
      compression_level = compression_level,
      preemptible_tries = if data_too_large_for_preemptibles then 0 else papi_settings.agg_preemptible_tries
  }
  ...
  if (defined(haplotype_database_file)) {
    ...
    call QC.CrossCheckFingerprints as CrossCheckFingerprints {
      input:
        ...
        metrics_filename = sample_info.base_file_name + ".crosscheck",
  ...
  call Processing.CheckContamination as CheckContamination {
    input:
      ...
      output_prefix = sample_info.base_file_name + ".preBqsr",
      preemptible_tries = papi_settings.agg_preemptible_tries,
  ...
  scatter (subgroup in CreateSequenceGroupingTSV.sequence_grouping) {
    ...
    call Processing.BaseRecalibrator as BaseRecalibrator {
      input:
        input_bam = SortSampleBam.output_bam,
        input_bam_index = SortSampleBam.output_bam_index,
        recalibration_report_filename = sample_info.base_file_name + ".recal_data.csv",
  ...
  call Processing.GatherBqsrReports as GatherBqsrReports {
    input:
      ...
      output_report_filename = sample_info.base_file_name + ".recal_data.csv",
      preemptible_tries = papi_settings.preemptible_tries
  }
  ...
  call Processing.GatherSortedBamFiles as GatherBamFiles {
    input:
      ...
      output_bam_basename = sample_info.base_file_name,
      total_input_size = agg_bam_size,
      compression_level = compression_level,
      preemptible_tries = papi_settings.agg_preemptible_tries
  }

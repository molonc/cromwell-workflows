...
task MarkDuplicates {
  ...
  command {
   # increased java memory size and max memory size
    java -Dsamjdk.compression_level=~{compression_level} -Xms~{increased_java_memory_size}g -Xmx~{increased_max_java_memory_size}g -jar /usr/gitc/picard.jar \
      MarkDuplicates \
...
task BaseRecalibrator {
  ...
  Int disk_size = ceil(size(input_bam, "GB") + size(input_bam_index, "GB") + ref_size + dbsnp_size) + 50
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "18 GB" # increase from 6 -> 18 (2025-01-14)
    disk: disk_size + " GB"
  }
  output {
...
task GatherSortedBamFiles {
  ...
  Int disk_size = ceil(4 * total_input_size) + 50 #20 -> 50
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "24 GB" #increase from 3 -> 6 (2025-01-14) -> 20 (2025-08-25)
    disk: disk_size + " GB"
  }
  output {
    File output_bam = "~{output_bam_basename}.bam"
...
task GatherUnsortedBamFiles {
  ...
  Int disk_size = ceil(4 * total_input_size) + 200 #increase from 2 -> 3 -> 4 and 20 -> 100 -> 150 -> 200 (2025-09-15)
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "16 GB" #increase from 3 -> 6 -> 12 -> 16 (2025-09-15)
    disk: disk_size + " GB"
  }
  output {
    File output_bam = "~{output_bam_basename}.bam"

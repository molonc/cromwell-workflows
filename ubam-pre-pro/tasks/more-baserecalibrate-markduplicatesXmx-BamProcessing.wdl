...
task MarkDuplicates {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xms~{java_memory_size}g -Xmx~{max_java_memory_size}g -jar /usr/gitc/picard.jar \
      MarkDuplicates \
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "67.5 GB" # increased from ~{memory_size}, which would've been 22.5 GB
    disk: disk_size + " GB"
  }
  output {
    File output_bam = "~{output_bam_basename}.bam"
...
task GatherUnsortedBamFiles {
  ...
  # Multiply the input bam size by two to account for the input and output
  Int disk_size = ceil(3 * total_input_size) + 100 #increase from 2 -> 3 and 20 -> 100 (2025-01-14)

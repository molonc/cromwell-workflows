...
task SortSam {
  ...
  runtime {
    ...
    cpu: "4" # increase from 1 -> 2 (04/14/2023) -> 4
    memory: "14000 MB" # increase from 5000 MB -> 7000 MB (04/14/2023) -> 14000
    preemptible: true
...
task MarkDuplicates {
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "~{increased_memory_size} GB" # memory_size -> increased_memory_size
    disk: increased_disk_size + " GB" # disk_size -> increased_disk_size
  }
  output {
    File output_bam = "~{output_bam_basename}.bam"
...
task ApplyBQSR {
  input {
    ...
    Int preemptible_tries
    String gatk_docker = "us.gcr.io/broad-gatk/gatk:4.0.10.1"
    Int memory_multiplier = 3 # 1 -> 3; 2025-09-25
    Int additional_disk = 40 # 20 -> 40; 2025-09-25              
  }
...
task GatherUnsortedBamFiles {
  ...
  # Multiply the input bam size by four to account for the input and output
  Int disk_size = ceil(4 * total_input_size) + 200 # (50 -> 200 on 2025-09-24)
  ...
  runtime {
    ...
    preemptible: true
    maxRetries: preemptible_tries
    memory: "20 GB" # (15 -> 20 on 2025-09-24)
    disk: disk_size + " GB"
  }
  output {
    File output_bam = "~{output_bam_basename}.bam"

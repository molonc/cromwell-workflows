...
workflow MarkDuplicates {
    ...
    call IndexBam {
        input: 
            sorted_bam = MarkDuplicates.output_bam, 
            output_file_name = sample_name + "_no_duplicates.bai"
    }

    output {
        File outfile = MarkDuplicates.output_bam
...
task MarkDuplicates {
  ...
  command <<<
    java -Dsamjdk.compression_level=~{compression_level} -Xms~{java_memory_size}g -jar /usr/gitc/picard.jar \
      ...
      OUTPUT=~{sample_name + "_no_duplicates"}.bam \
  ...
  output {
    File output_bam = "~{sample_name + "_no_duplicates"}.bam"
  }
}

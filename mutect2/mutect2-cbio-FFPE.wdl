...
## - One analysis-ready BAM file (and its index and ID) for each sample
##
...
## 
## LAST UPDATED: August 22, 2023 - increased runtime metrics for M2 task (Kelly Zhang)
...
workflow Mutect2 {
    input {
      ...
      Float large_input_to_output_multiplier = 3.0
    ...
    if (defined(realignment_index_bundle)) {
        ...
        call FFPEFilter {
            input:
                vcf = FilterAlignmentArtifacts.filtered_vcf,
    ...
    if (run_funcotator_or_default) {
        File funcotate_vcf_input = select_first([FFPEFilter.outfile, Filter.filtered_vcf])
        File funcotate_vcf_input_index = select_first([FFPEFilterIndex.outfile, Filter.filtered_vcf_idx])
        call Funcotate {
            input:
                ref_fasta = ref_fasta,
                ref_fai = ref_fai,
        ...
        if (run_maf_formatting) {
            call CbioMaf.CbioExtMafFormatter {
                input:
                    input_maf = MafFuncotate.maf_funcotated_output_file,
                    maf_id = tumor_id,
                    normal_id = normal_id
            }
        }
    ...
    output {
        ...
        File? cbio_maf = CbioExtMafFormatter.output_maf_file
    }
}
...
task M2 {
    ...
    Int machine_mem = if defined(mem) then mem * 2000 else 8192 # used to be: if defined(mem) then mem * 1000 else 3500
    ...
    runtime {
        ...
        disk: select_first([disk_space, 650]) + " GB" # select_first([disk_space, 100]) + " GB"
        preemptible: true
        maxRetries: select_first([max_retries, 0])
        cpu: select_first([cpu, 8]) # select_first([cpu, 1])
    }
